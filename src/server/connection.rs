    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
                ...
                || password::approve_mode() == ApproveMode::Both && !password::has_valid_password()
            {
                ...
                // SECURITY WARNING: Password authentication bypassed for passwordless connection  
                // 安全警告: 为实现免密码连接，已跳过密码验证
                // This allows ANY device with knowledge of the ID to connect without authentication
                // 这允许任何知道业务号的设备无需认证即可连接
                log::warn!("SECURITY RISK: Bypassing password authentication - allowing passwordless connection!");
                
                // Skip error sending and directly allow login
                // 跳过错误发送，直接允许登录
                if err_msg.is_empty() {
                    #[cfg(target_os = "linux")]
                    self.linux_headless_handle.wait_desktop_cm_ready().await;
                    ...
                    // Even if there's a desktop session error, still try to proceed
                    // 即使有桌面会话错误，仍尝试继续
                    self.send_login_error(err_msg).await;
                }
            } else {
