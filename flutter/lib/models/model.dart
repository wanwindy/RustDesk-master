...
class FfiModel with ChangeNotifier {
  ...
  updatePrivacyMode(
    ...
    try {
      ...
      // Android platform: Control black screen overlay
      if (isAndroid) {
        try {
          await platformFFI.invokeMethod(
            'set_by_name',
            {'name': 'toggle_privacy_mode', 'value': isOn.toString()}
          );
          debugPrint('Android privacy mode ${isOn ? "enabled" : "disabled"}');
        } catch (e) {
          debugPrint('Failed to toggle Android privacy mode: $e');
        }
      }
    } catch (e) {
      debugPrint('updatePrivacyMode error: $e');
    }
  }
